...
var piezas2 = [0,1,2,3,4,5,6,7,8]
var piezas3 = [0,1,2,3,4,5,6,7,8]
...
var todo = [piezas, piezas2]
var bandera = Math.random() 
console.log(todo)

if(bandera > 0,5) {
    todo.pop()
    for (let i=0; i<8; i++) {
        todo = todo["imagenes_nivel2/A"+i+".png"];
        console.log(todo)
    }
} else {
    todo.shift()
    for (let i=0; i<8; i++) {
        todo = todo["imagenes_nivel2/B"+i+".png"];
        console.log(todo)
    }
}
...
function desordenar() {
    todo[0] = todo[0].sort(() => {return Math.random() -0.5})
    //console.log(piezas)
}
